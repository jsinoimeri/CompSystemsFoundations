...
print2Str:
					...
					LoopPS2:
										MOV AL, [SI]	; Load the next char to be printed - USING INPUT PARAMETER BP
										CMP AL, '$'		; Compare the char to '$'
										JE quitPS2		; If it is equal, then quit subroutine and return to calling code
										
										OUT DX,AL			; If it is not equal to '$', then print it
										ADD SI, 2			; Point to the next char to be printed
...
main:
			...
			MOV AL, '$'											; Copy the ascii value of $ to AL
			MOV AH, 0												; Copy the value 0 to AH
			...
			gsPrint:
								MOV SI, SP						; Load the starting address of the string
								CALL print2Str				; Print the string in reverse
								CALL 	newLine					; Print new Line
