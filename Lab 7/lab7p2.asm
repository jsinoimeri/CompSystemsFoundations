...
Message1: .DB	'Enter an even number between [0-9]: $'		; User prompt message
Message2: .DB 'Congrads it is a valid even number. $'		; Congrads message when even number entered
...
main:
			...
			mov DX, DISPLAY					; Copy Display value into DX
			out DX, AL							; Echo the character back to the display	
			
			CMP AL, 30h							; Check if AL is 0
			JBE main									; If so loop to main
			
			CMP AL, 39h
			JA main
			
			SHR AL, 1								; Right shift AL by 1
			JC main									; Jump if carry to main
			
			MOV SI, Message2				; Load starting address of Message2 to SI
			
			Call newLine						; Print a new line
			Call printStr						; Call prtstr to print Message2
